...
<div class="container-fluid">
	<div
		...
		<form action="" id="productForm" method="post">
			...
			<c:if test="${dto.convertToCustomerEnable=='Y' && dto.convertToCustomerWebEnable=='Y' }">
				<div class="row buttonHeading exis-fac-row">
					<div class="col-lg-9 col-md-9 col-sm-6 col-xs-5">
					...
					<div class="bs-example">
						...
						<table class="table table-striped margin-table-bottom-space">
							...
							<tbody>
								<tr>
									<td></td>
									<td><span class="sel-ttip"> <select id="branch" disabled="disabled"
											name="branchId" style="width: 140px; margin-top: 6px;">
												<option value="-1">select</option>
												<c:forEach var="obj"
													items="${productMasterDetail.branchMaster}">
									...
									<td><span class="sel-ttip"> <select
											...
											style="width: 145px; margin-top: 7px" disabled="disabled">
												<option value="-1">select</option>
												<c:forEach var="obj"
									...
									<td><span class="sel-ttip"> <select
											name="schemeId" id="selectfacility"
											onchange="hideText(this)"
											style="width: 145px; margin-top: 7px" disabled="disabled">
												<option value="-1">select</option>
												<c:forEach var="facility" items="${productMasterDetail.productMaster }">
													<c:if test="${facility.prodTypeId=='1000000002'}">
														<option value="${facility.prodId}" ${facility.prodId==productDetail.schemeId?'Selected':'' }>${facility.prodName}</option>
													</c:if>
												</c:forEach>
										</select>
									</span> <span class="select-tooltip"
									...
									<td><input type="text" class="rgtAlign"
										...
										id="facilityRequestedLoanAmount"
										style="width: 145px; margin-top: 7px"
										onkeypress="return onlyNumeric(event);" maxlength="10" readonly="readonly" disabled="disabled"/>
																				
										<input type="hidden" name="hiddenLeadId" id="hiddenLeadId" value="${productDetail.hiddenLeadId}" readonly="readonly" disabled="disabled"/>
									...
									<td><input type="text" class="rgtAlign"
										...
										name="facilityRequestedTenor" style="width: 40px"
										id="facilityRequestedTenor"
										onkeypress="return onlyNumeric(event);" readonly="readonly" disabled="disabled"/></td>
									...
									<td><c:if test="${not empty dto.prospectId}">
											...
											<button type="button"
												class="btn btn-primary convertbtn_prodrefferral"
												onclick="javascript:validate();" disabled="disabled">Convert to
												Customer</button>
			...
			<div class="row buttonHeading prod-reff">
				...
				<div class="col-lg-3 col-md-3 col-sm-6 col-xs-7 btnPosition">
					<button type="button" class="btn btn-primary"
						onclick="deleteXSell();" disabled="disabled">Delete</button>
					<button type="button"
						class="btn btn-primary add-list-prodreffereal" id="prodreffereal"
						value="	${xSellList eq null ? '0' : fn:length(xSellList)}" disabled="disabled">Add</button>
				...
				<div class="bs-example productreferral-table">
					...
					<table class="table table-striped tr-padding-zero">
						...
						<tbody>
							...
							<c:forEach var="xSellData" items="${xSellList}"
								...
								<tr>
									<td><input type="checkbox" value="${xSellData.xsellId }"
										...
										id="listXSell[${xSellStatus.index }].xsellId" disabled="disabled"/></td>
									<td style="position: relative;"><select
										disabled="disabled" name="listXSell[${xSellStatus.index }].facilityReqId"
									...
									<td><select disabled="disabled" name="listXSell[${xSellStatus.index }].bankId"
									...
									<td><input disabled="disabled" type="text" value="${xSellData.amount }"
							...
							<c:if test="${xSellData.isXsold=='N'}">javascript:void(0)</c:if>'>Convert</a></td>
